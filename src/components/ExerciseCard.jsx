...
export default function ExerciseCard(props) {
  ...
  return (
    <div className='p-4 rounded-md  flex flex-col gap-4  bg-salte-950 sm:flex-wrap'>
        ...
        <div className='flex flex-col'>
            <h3 className='text-salte-400 text-sm '> Muscle Group</h3>
            ...
            {['reps','rest','tempo',''].map(info => {
                return(
                    <div  key={info} className='flex flex-col p-2 rounded border-[1.5px] border-solid  border-slate-900 w-full '>
                        <h3 className='captilize  text-slate-400 text-sm '>{info === 'reps' ? `${exercise.unit}` : info}</h3>
